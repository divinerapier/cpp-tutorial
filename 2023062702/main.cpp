...
int main() {
    // 错误代码:
    // 使用 malloc 在堆上，为对象申请内存时，仅分配了内存空间，不会调用 **任何构造函数**
    // 所以在使用 cpp 开发时，不要使用 malloc 为 cpp 的类动态分配内存
    //      Cat *felinae = (Cat *) malloc(sizeof(Cat));

    // 正确的动态分配方式:
    //      new 操作符会将对上创建在 堆 内存区域，该内存区域需要开发者手动管理(释放)
    Cat *ragdoll = new Cat("Ragdoll");

    // 错误释放方法
    // 当使用 new 操作符创建的堆对象，不可以使用 free 释放 (没有调用析构函数)
    //  free(ragdoll);
